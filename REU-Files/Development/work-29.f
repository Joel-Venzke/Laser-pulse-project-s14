...
!
      if (setA.eq.'f') then  ! We are actually setting the E-field
       do i = 0,ntfinal
         ...
         efield3(i) = alph3*ee3*envelope3(i)*sin(ww3*timeval(i)+cep3rad)
         efieldtot(i) = efield1(i)+efield2(i)+efield3(i)
       end do
      else  ! We set A (the default case)
       do i = 0,ntfinal
        afield1(i) = envelope1(i)*sin(ww1*timeval(i)+cep1rad)
        afield2(i) = envelope2(i)*sin(ww2*timeval(i)+cep2rad)
        afield3(i) = envelope3(i)*sin(ww3*timeval(i)+cep3rad)
       end do
