      module parameters
      parameter(nxmax=500200,ncmax=99,nenmax=10000,nfmax=37)
     ...
     1                 w2,e2,s2,ton2,toff2,del2,agbr 
      ...
      double precision, allocatable  :: afield1(:),afield2(:)
      double precision, allocatable  :: efield1(:),efield2(:)
...
C     open(101,file='spectra.out',status='unknown')
...
! new version of subroutine pulse by Klaus; written January 2014
     ...
     >                 w(1001),greal(1001),gimag(1001)
      double precision efield1max,efield2max,e1norm,e2norm
...
!  set defaults for second pulse 
      ...
      rr2        = 0.0d0
      cep2       = 90.0d0
      x2up       = 2*x1up       ! consistent with second harmonic
      x2plat     = 2*x1plat
      x2down     = 2*x1down
      shape2up   = 's'
      shape2down = 's'
...
! allocate the arrays
     ...
     >         envelope2(0:ntfinal),afield1(0:ntfinal),
     >         afield2(0:ntfinal),afieldtot(0:ntfinal),
...
!
      if (setA.ne.'t') then  ! We are actually setting the E-field
       do i = 0,ntfinal
         efield1(i) = alph1*ee1*envelope1(i)*sin(ww1*timeval(i)+cep1rad)
         efield2(i) = alph2*ee2*envelope2(i)*sin(ww2*timeval(i)+cep2rad)
         efieldtot(i) = efield1(i)+efield2(i)
       end do
      else  ! We set A (the default case)
       do i = 0,ntfinal
        afield1(i) = alph1*ee1*envelope1(i)*sin(ww1*timeval(i)+cep1rad)
        afield2(i) = alph2*ee2*envelope2(i)*sin(ww2*timeval(i)+cep2rad)
       end do
...
1000  format('#   time (a.u.)   time(fs)      ',
     ...
     >   envelope1(i),envelope2(i),efield1(i),efield2(i),efieldtot(i)
      end do 
